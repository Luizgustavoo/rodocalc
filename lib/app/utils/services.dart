...
class Services {
  ...
  static int reaisParaCentavos(String valorEmReais) {
    try {
      // Remove o prefixo "R$" e possíveis espaços
      String valorLimpo =
          valorEmReais.replaceAll(RegExp(r'[^\d,]'), '').replaceAll(',', '.');

      // Converte a string limpa para double
      double valor = double.parse(valorLimpo);

      // Multiplica o valor por 100 e usa o método round() para arredondar para o inteiro mais próximo
      return (valor * 100).round();
    } catch (e) {
      // Retorna 0 se houver um erro na conversão
      print('Erro na conversão: ${e.toString()}');
      return 0;
    }
  }
}
